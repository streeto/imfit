...
#ifdef DEBUG
  if (CheckFinite(m*nfree, fjac)) {
    printf("*mpfit: fjac is finite\n");
  } else {
    printf("*mpfit: fjac is NOT finite!\n");
  }
#endif
  if (iflag < 0) {
    goto CLEANUP;
...
#ifdef DEBUG
    printf("  *mp_fdjac2: j=%d, temp=%g, h=%g\n", j, temp, h);
#endif
    x[ifree[j]] = temp + h;
...
#ifdef DEBUG
    if (CheckFinite(m, fvec)) {
      printf("  *mp_fdjac2: fvec vector is finite\n");
    } else {
      printf("  *mp_fdjac2: fvec vector is NOT finite!\n");
    }
    if (CheckFinite(m, wa)) {
      printf("  *mp_fdjac2: wa vector is finite\n");
    } else {
      printf("  *mp_fdjac2: wa vector is NOT finite!\n");
    }
#endif
    if (dsidei <= 1) {
